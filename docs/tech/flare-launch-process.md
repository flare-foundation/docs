...
* **FIP.01**: [A governance proposal](https://proposals.flare.network/FIP/FIP_1.html) that, among other things, changes the initial token distributions as explained below.
...
It is worth summarizing the two current token distribution plans, as only one of them will be implemented depending on whether [FIP.01](https://proposals.flare.network/FIP/FIP_1.html) is approved or not.
...
* Are not rewarded. [All inflation is burned during observation mode](https://flare.network/om-inflation/).
...
The [FIP.01 proposal](https://proposals.flare.network/FIP/FIP_1.html) modifies how the rest of the tokens (after the TDE) are to be distributed, so it needs to be voted on by the community.
...
All FLR token holders (obtained either from the new airdrop or bought at Exchanges) can vote on [FIP.01](https://proposals.flare.network/FIP/FIP_1.html) using a voting front-end (under development).
...
The changes proposed in [FIP.01](https://proposals.flare.network/FIP/FIP_1.html) **are implemented**.
