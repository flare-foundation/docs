...
This is equivalent to using [`deposit`](#fn_deposit) followed by [`transfer`](#fn_transfer).
...
A proxy for the [`deposit`](#fn_deposit) method.
...
through ERC-20's [`approve`](#fn_approve) mechanism.

Reverts if `_amount` is higher than `_owners`'s `WNAT` balance or than
`msg.sender`'s [`allowance`](#fn_allowance) over `_owner`'s tokens.
